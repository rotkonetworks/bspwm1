...
The effects a rule may carry are: 'monitor=MONITOR_SEL', 'desktop=DESKTOP_SEL',
'node=NODE_SEL', 'state=STATE', 'layer=LAYER',
'honor_size_hints=(true|false|tiled|floating)', 'split_dir=DIR',
'split_ratio=RATIO', 'rectangle=WxH+X+Y' and the booleans
...
*shift*, *mod3* and *mod5*. The key is an XKB keysym name and is matched
unshifted, so write *"super + shift + q"*, not *"super + Q"*. Num Lock, Caps
Lock and Scroll Lock are ignored when matching; *mod2* is conventionally Num
Lock, so a binding that names it could never fire and is refused.
